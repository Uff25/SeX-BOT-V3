module.exports = {
	...
	onStart: async ({ threadsData, event, api }) => {
		...
		try {
			...
			console.warn(`[autoUpdateThreadInfo] Failed to load thread ${threadID}: ${error.message}`);
			return;
		}
		if (!threadInfo) {
			console.warn(`[autoUpdateThreadInfo] Thread data unavailable for ${threadID}`);
			return;
		}
		...
		switch (logMessageType) {
			case "log:subscribe":
				return async function () {
					...
					try {
						...
						console.warn(`[autoUpdateThreadInfo] Failed to refresh thread ${threadID}: ${error.message}`);
						return;
					}
					if (!threadInfo_Fca)
						return;
			...
			case "log:thread-name":
				return async function () {
					...
					if (!threadName || threadInfo.threadName === threadName)
						return;
					await threadsData.set(threadID, threadName, "threadName");
					await api.sendMessage(`✅ Group name changed.\nNew name: ${threadName}`, threadID);
				};
