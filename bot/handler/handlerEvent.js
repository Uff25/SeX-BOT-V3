    ...
    return async function (event, message) {
        ...
        if (body && body.trim() === prefix) {
            return message.reply(
                `<🎀>𝐇𝐞𝐲, ${senderName}! 𝐈 𝐧𝐨𝐭𝐢𝐜𝐞𝐝 𝐲𝐨𝐮 𝐭𝐲𝐩𝐞𝐝 𝐭𝐡𝐞 𝐩𝐫𝐞𝐟𝐢𝐱. 𝐓𝐲𝐩𝐞 ${prefix}𝐡𝐞𝐥𝐩 𝐭𝐨 𝐬𝐞𝐞 𝐚𝐥𝐥 𝐜𝐨𝐦𝐦𝐚𝐧𝐝𝐬.`
            );
        }
        ...
        const parameters = {
            ...
            envEvents, envGlobal, role, isAdmin,
        ...
        async function handlerEvent() {
            ...
            for (const [key, getEvent] of allEventCommand) {
                ...
                const time = getTime("DD/MM/YYYY HH:mm:ss");
                try {
                    const result = await getEvent.onStart({
                        ...parameters,
                        commandName,
                        getLang: getText2
                    });
                    if (typeof result === "function")
                        await result();
                    log.info("EVENT COMMAND", `Event: ${commandName} | ${author} | ${userData?.name || "Unknown User"} | ${threadID}`);
                }
                catch (err) {
