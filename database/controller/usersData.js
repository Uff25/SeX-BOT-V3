...
module.exports = async function (databaseType, userModel, api, fakeGraphql) {
                                ...
                                switch (databaseType) {
                                                                case "mongodb": {
                                                                                                ...
                                                                                                Users = (await userModel.find({}).batchSize(100).maxTimeMS(60000).lean()).map(user => _.omit(user, ["_id", "__v"]));
                                                                                                break;
                                                                }
                                                                case "sqlite": {
                                  ...
                                  async function getAvatarUrl(userID) {
                                                                if (isNaN(userID)) {
                                                                  ...
                                                                  try {
                                                                                                  ...
                                                                                                  const directURL = user.data?.data?.profile?.profile_picture?.uri;
                                                                  ...
                                                                  if (api && typeof api.getAvatarUser === "function") {
                                                                                                  try {
                                                                                                                                 const avatarData = await api.getAvatarUser(userID);
                                                                                                                                 const avatarURL = avatarData?.[String(userID)];
                                                                                          ...
                                                                                          if (api && typeof api.getUserInfo === "function") {
                                                                                                  try {
                                                                                                                                 ...
                                                                                                                                 const profile = userInfo?.[String(userID)] || userInfo?.[userID] || {};
                                                                                                                                  const avatarURL = profile.profilePicUrl || profile.thumbSrc;
                                                                                                                                   if (avatarURL) return avatarURL;
                                                                                                  }
                                                                                                  catch (err) {}
                                                                                          }

                                                                                           const token = api?.getContext?.()?.access_token || process.env.FACEBOOK_GRAPH_ACCESS_TOKEN;
                                                                                          if (!token) return null;
                                                                                           const params = new URLSearchParams({ height: "2048", width: "2048", access_token: token });
                                                                                          return `https://graph.facebook.com/${userID}/picture?${params}`;
                                 }
